    ...
    <body class="bg-gray-100">
        <header class="p-5 border-b bg-white shadow">
            <div class="container mx-auto flex justify-between items-center">
                ...
                @auth
                    <a class="flex items-center gap-2 bg-white border p-2 text-gray-600 rounded text-sm font-bold cursor-pointer" href="{{route('posts.create')}}">
                    ...
                    <form action="{{route('logout')}}" method="POST">  
                        @csrf
                        <button type="submit" class="font-bold uppercase text-gray-600 text-sm">Cerrar Sesíon</button>
                    </form>
                @endauth
                @guest
